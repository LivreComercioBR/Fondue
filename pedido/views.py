...
def finalizar_pedido(request):
    if request.method == "GET":
        ...
        return render(request, 'finalizar_pedido.html', {'carrinho': len(request.session['carrinho']), 'categorias': categorias, 'total': total, 'erro': erro})
    else:
        if len(request.session['carrinho']) > 0:
            x = request.POST
            total = sum([float(i['preco'])
            ...
            def lambda_func_troco(x): return int(
                x['troco_para'])-total if not x['troco_para'] == '' else ""

            def lambda_func_pagamento(
                x): return 'Cartão' if x['meio_pagamento'] == '2' else 'Dinheiro'
            ...
            return redirect('/finalizar_pedido?erro=1')


def validacupom(request):
    return HttpResponse("Opa, você ganhou um excelente desconto!")
